...
let%expect_test "Test elab 3" =
  ...
  [%expect{|
    ...
    proc empty_queue (s : {'ins : %tp_2 & 'del : %tp_4}) [] = recv s ('ins => recv s (x => t : {'ins : %tp_2 & 'del : %tp_4} <- (call empty_queue (t) []); call node_queue (s) [x, t]) | 'del => send s 'none; send s ())
    proc node_queue (s : {'ins : %tp_2 & 'del : %tp_4}) [x : {'b0 : bin + 'b1 : bin + 'e : %tp_0}, t : {'ins : %tp_2 & 'del : %tp_4}] = recv s ('ins => recv s (y => ss : {'ins : %tp_2 & 'del : %tp_4} <- (call node_queue (ss) [x, t]); call node_queue (s) [y, ss]) | 'del => send t 'del; recv t ('none => recv t (() => send s 'some; send s x; call empty_queue (s) []) | 'some => recv t (y => send s 'some; send s y; call node_queue (s) [x, t]))) |}]
;;
